...
const Homeapplaince=()=>
{
  ...
  const Globlevalue=Globleselector.Catpage_routing.Cat_Name;
    ...
    const[homeCat,setCategories]=useState([])
    ...
    const fixArray=async()=>
    {
        ...
        .get("http://localhost:3000/Home")
          .then((response) => sethomeArr(response.data))
    ...
    const filtersFunctions=()=>
      {
        let Filterarry=homeArr.map((ele)=>ele);
    ...
    const Mainmenu=()=>
    {
        ...
        {(filterArray.length===0?homeArr:filterArray).map((ele,ind)=>{
           return(
            <>
