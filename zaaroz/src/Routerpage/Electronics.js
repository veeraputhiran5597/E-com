...
const Electronics=()=>
{
    ...
    const[electCat,setCategories]=useState([])
    ...
    const fixArray=async()=>
    {
        ...
        .get("http://localhost:3000/Electronics")
          .then((response) => setelectronicsArr(response.data))
    ...
    const filtersFunctions=()=>
      {
        let Filterarry=electronicsArr.map((ele)=>ele);
    ...
    const Mainmenu=()=>
    {
        ...
        {(filterArray.length===0?electronicsArr:filterArray).map((ele,ind)=>{
            return(
                <>
